...
export const bucketService = {
  ...
  reorderBuckets: async (
    projectId: string | number,
    bucketIds: (number | string)[],
  ): Promise<{ status: string; order: (number | string)[] }> => {
    return await apiFetch<{ status: string; order: (number | string)[] }>(
      `/projects/${projectId}/buckets/reorder`,
      {
        method: "PUT",
