    ...
    <>
          ...
          {loading ? (
            ...
            alerts.map((alert) => (
              <div
                ...
                className={`p-4 rounded-xl bg-[#1F2937] border group cursor-pointer transition-colors flex justify-between items-center ${alert.severity === 'critical' ? 'border-[#EF4444]/30 hover:border-[#EF4444]' : 'border-[#F59E0B]/30 hover:border-[#F59E0B]'
                  }`}
              >
                <div>
                  ...
                  <p className="text-slate-400 text-[10px] mt-0.5">{getProjectName(Number(alert.project_id))}</p>
                </div>
      ...
      {selectedAlert && (
        <AlertDetailModal
          ...
          onNavigate={() => onNavigateProject(Number(selectedAlert.project_id))}
