    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full">
        <div className="lg:col-span-4">
          <SurfaceCard title="Tasks at Risk" subtitle="Detect tasks to nudge" rightElement={<Badge variant="primary">{tasksAtRisk.length} Detected</Badge>} className="h-full">
             <div className="space-y-3 flex-1 overflow-y-auto no-scrollbar max-h-[300px] pr-1">
                ...
                {tasksAtRisk.length === 0 && <p className="text-slate-500 text-[11px] text-center py-4">All tasks on track.</p>}
             </div>
             {tasksAtRisk.length > 0 && <Button variant="outline" className="w-full mt-4 !text-[10px] !py-2 bg-[#1F2937]">Nudge Selected</Button>}
          </SurfaceCard>
        </div>
        <div className="lg:col-span-8">
          <SurfaceCard title="Stagnation Radar" subtitle="Granular Bottleneck Analysis" icon={TrendingDown} className="h-full">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8 mt-2">
                ...
                {metrics.length === 0 && <div className="col-span-2 text-slate-500 text-[12px] text-center py-8">No metrics available.</div>}
             </div>
          </SurfaceCard>
        </div>
      </div>
...
export const ProjectOverviewDev: React.FC<ProjectOverviewProps> = ({ projectId }) => {
  ...
  const activeTask = tasks.find(t => t.status === 'ONGOING' && t.lead_assignee_id === 1); // Mock lead is 1
  const myReviewTasks = tasks.filter(t => t.status === 'ON REVIEW' && t.lead_assignee_id === 1);
