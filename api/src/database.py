...
def postgresql_dsn():
    ...
    # Full URL preference (best for Supabase transaction/session pool URLs)
    url = getattr(settings, "postgresql_database_url", None)

    # If full URL points to direct IPv6-only Supabase DB and pooler endpoint exists,
    # prefer pooler components to avoid network timeout on IPv4-only environments.
    if url:
        try:
    ...
    if url:
        # ensure sslmode is present for Supabase-hosted Postgres
        try:
            parts = urlparse(url)
            ...
            # if parsing fails, fall back to returning the original url
            pass
        return url
    ...
    if not all([user, pwd, host, db]):
        raise RuntimeError(
            "No Postgres configuration available: set POSTGRESQL_DATABASE_URL or the component vars (POSTGRESQL_USERNAME/POSTGRESQL_PASSWORD/POSTGRESQL_HOST/POSTGRESQL_DATABASE)"
        )

    # Direct Supabase DB hosts are often IPv6-only; if this machine has no IPv6 route,
    # fail fast with an actionable message instead of hanging on connection timeout.
...
def db_create_user(user: DatabaseUser):
    ...
    try:
        ...
        # Prepare columns and params only for provided values
        mapping = {
